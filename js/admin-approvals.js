...
async function handleApproval(approvalId, action) {
    ...
    const confirmMessage = action === 'approve' 
        ? `Approve ${approval.first_name} ${approval.last_name} as ${approval.role}?`
        : `Reject ${approval.first_name} ${approval.last_name}'s ${approval.role} request?`;
    ...
    try {
        ...
        if (result.success) {
            showNotification(action === 'approve' ? 'Approved successfully' : 'Rejected', action === 'approve' ? 'success' : 'info');
            fetchPendingApprovals();
        } else {
